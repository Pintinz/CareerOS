...
from app.services.question_import_service import import_interview_questions
...
async def create_question_admin(
    ...
    question = await InterviewService(db).admin_create_question(payload, admin.id)
    await audit_service.record(db, admin_id=admin.id, action="create", entity_type="interview_question", entity_id=question.id)
    return question
...
async def update_question_admin(
    question_id: str, payload: InterviewQuestionUpdate, db: AsyncSession = Depends(get_db), admin: AdminUser = Depends(get_current_admin)
) -> InterviewQuestionAdminOut:
    question = await InterviewService(db).admin_update_question(question_id, payload)
    await audit_service.record(db, admin_id=admin.id, action="update", entity_type="interview_question", entity_id=question_id)
    return question
...
async def delete_question_admin(
    ...
    await audit_service.record(db, admin_id=admin.id, action="delete", entity_type="interview_question", entity_id=question_id)
...
async def bulk_import_questions_admin(
    ...
    """CSV columns: question_text, category_slug, topic_slug, field, industry, job_role,
    difficulty, evaluation_points (pipe-`|`-separated), follow_up_prompt."""
    csv_text = (await file.read()).decode("utf-8-sig")
    result = await import_interview_questions(db, csv_text, admin_id=admin.id)
    await audit_service.record(
        db, admin_id=admin.id, action="bulk_import", entity_type="interview_question",
