...
import { QuestionAdminOut, QuestionCategoryOut } from "@/types/questionModels";

export default function AptitudeQuestionsPage() {
  const { checked } = useAdminGuard();
  const [items, setItems] = useState<QuestionAdminOut[]>([]);
  ...
  async function load() {
    try {
      ...
      const res = await api.get<PaginatedResponse<QuestionAdminOut>>(`/admin/aptitude/questions?page=1&page_size=100${query}`);
      setItems(res.items);
    } catch (err) {
  ...
  async function handleDelete(q: QuestionAdminOut) {
    ...
    await api.delete(`/admin/aptitude/questions/${q.id}`);
    await load();
  }
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:8000/api/v1"}/admin/aptitude/questions/bulk-import`, {
      method: "POST",
          ...
          <tbody>
            ...
            {items.map((q) => (
              <tr key={q.id} className="border-b border-line last:border-0">
                ...
                <td className="px-4 py-3 text-muted">{q.question_type}</td>
