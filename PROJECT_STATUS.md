...
- Branch `career-source-integration` (from `live-opportunity-discovery`): `c1f33a6` provider adapters (Workday scoping, Oracle Recruiting, microdata/sitemaps), `ce8fd3f` registry, seed pack and sync policy, `8f976e5` fixes from the live sync, `c5b3b80` admin/mobile, then docs — see **CAREER_SOURCE_INTEGRATION.md**.
...
| Direct Company Career Feed Integration | IN PROGRESS (live-synced into a dev database; not yet polling in production) | Starter pack of 74 audited official career sources (21 ready via public ATS/APIs, 5 via official pages, 25 need configuration, 23 manual-only or blocked), Workday country facets, new Oracle Recruiting adapter, SuccessFactors microdata and sitemap reading, per-source country scope, removal confirmation (POSSIBLY_REMOVED), auto-draft fast path, Test connection and health, region/function filters, verification labels. See CAREER_SOURCE_INTEGRATION.md and docs/career_sources.md. |
